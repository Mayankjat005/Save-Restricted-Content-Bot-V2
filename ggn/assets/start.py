...
async def start(event):
    ...
    collection.update_one({"user_id": user_id}, {"$set": {"user_id": user_id}}, upsert=True)
    buttons = [
        [Button.url("Join Channel", url="https://t.me/WarriorUnitsBots")],
        [Button.url("Contact Me", url="https://t.me/oo7jatji")],
        [Button.url("Anime Channel", url="https://t.me/Warrior_Units")],
    ]
...
PRE_TEXT = """💰 **Premium Price**: Starting from $3 or 200 INR accepted via **__Amazon Gift Card__** (terms and conditions apply).
📥 **Download Limit**: Users can download up to 100 files in a single batch command.
🛑 **Batch**: You will get two modes /bulk and /batch.
🛑🛑🛑 First Contact to the Admin @oo7jatji then Send Gift Card 🛑🛑
...
async def help_command(event):
    buttons = [[Button.url("Buy Bot Source Code", url=REPO_URL)]]
...
async def youtube_dl_command(_, message):
    ...
    if len(message.command) > 1:
        ...
        try:
            ...
            caption = f"{video_info['title']}\n\n__**Powered by [Warrior Units](https://t.me/WarriorUnitsBots)**__"  # Set caption to the title of the video
